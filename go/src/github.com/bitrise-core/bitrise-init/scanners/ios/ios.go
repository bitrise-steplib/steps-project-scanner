...
func (scanner *Scanner) Options() (models.OptionModel, models.Warnings, error) {
	...
	for _, project := range cleanProjectFiles {
		...
		if isWorkspace {
			log.Info("Inspecting workspace file: %s", project)
		} else {
		...
		if isWorkspace {
			...
			log.Details("%d workspace shared scheme(s) detected", len(workspaceSchemeXCTestMap))
			for scheme := range workspaceSchemeXCTestMap {
			...
			if found {
				...
				for workspace, linkedProject := range podfileWorkspaceProjectMap {
					if linkedProject == project {
						log.Details("workspace (%s) will be generated by CocoaPods", workspace)
			...
			log.Details("%d project shared scheme(s) detected", len(projectSchemeXCtestMap))
			for scheme := range projectSchemeXCtestMap {
				log.Details("- %s", scheme)
			}
